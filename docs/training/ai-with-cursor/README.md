...
Capacitación para que desarrolladores usen Cursor (y un kit de contexto) al construir un producto. El alumno **trae un repositorio que ya existe**; no se genera un proyecto nuevo.
...
1. Usar Cursor con alcance: qué adjuntar, qué no leer, cómo verificar.
...
**Para dictar:** usa solo [`facilitator-runbook.md`](facilitator-runbook.md) (guión en español coloquial). Los archivos `01-session.md` … `06-session.md` son **anexo** (tarea de casa + checklist).

| Sesión | Guión | Capa | Resultado en el repo del alumno |
| --- | --- | --- | --- |
| 1 | [runbook → S1](facilitator-runbook.md#sesión-1--cursor-sin-kit) · [anexo](01-session.md) | Ninguna (skill) | Un cambio pequeño hecho con Cursor, sin carpeta `cursor/` |
...
1. Una **skill de Cursor** (modos, `@`, alcance, verificación).
...
- Cursor instalado y abierto en **su** repo (no un greenfield).
